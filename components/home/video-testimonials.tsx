...
export function VideoTestimonials() {
  ...
  const openVideo = (index: number) => setSelectedVideoIndex(index)
  const closeVideo = () => setSelectedVideoIndex(null)
  const nextVideo = () => {
  ...
  const previousVideo = () => {
    if (selectedVideoIndex === null) return
    setSelectedVideoIndex((selectedVideoIndex - 1 + VIDEO_TESTIMONIALS.length) % VIDEO_TESTIMONIALS.length)
  }
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-xl text-muted-foreground max-w-2xl mx-auto">{s.subtitle}</p>
            </div>
              ...
              <div className="overflow-hidden px-8 md:px-16" ref={emblaRef}>
                <div className="flex gap-6 md:gap-8">
                  {VIDEO_TESTIMONIALS.map((video, index) => (
                    <div key={video.id} className="flex-[0_0_70%] sm:flex-[0_0_45%] md:flex-[0_0_32%] lg:flex-[0_0_28%] min-w-0">
                      <button
                      ...
                      >
                        <video src={video.videoUrl} className="w-full h-full object-cover" muted playsInline preload="metadata" />
            ...
            <p className="text-center text-sm text-muted-foreground mt-8">{s.hint}</p>
          </div>
        </div>
      </section>
